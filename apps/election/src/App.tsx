...
export const App = () => {
  ...
  useEffect(() => {
    ...
    const tryReminder = () => {
      const now = new Date();
      const reminder = reminderTimes.find((item) => item.hour === now.getHours() && item.minute === now.getMinutes());
      if (!reminder) return;
